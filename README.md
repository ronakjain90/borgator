...
`/cost` breaks the session down by model — the manager and its workers are
priced separately, since they often run on different ones:

```
you /cost
Session cost — $0.61 estimated
  anthropic · claude-opus-4-8   —  in 118,204 · out 6,120    —  $0.74
  anthropic · claude-haiku-4-5  —  in 41,003 · out 2,880     —  $0.06
  estimated from published rates — check your provider for billing
```

Rates are USD per million tokens; cache reads bill at 0.1x the input rate and
cache writes at 1.25x. Only rates that can be stated accurately ship built in
(the Anthropic models, plus local providers at zero) — anything else reports
`rate unknown` rather than inventing a number. Add or correct any model in
`~/.borgator/pricing.json`:

```json
{ "openai": { "gpt-4o": { "input": 2.5, "output": 10 } } }
```

Entries merge over the built-ins per model and may set `cache_read` /
`cache_write` explicitly when a provider doesn't follow those multipliers.
`BORGATOR_PRICING_FILE` relocates the file.
...
| `~/.borgator/pricing.json` | Per-model rate overrides for `/cost` (`BORGATOR_PRICING_FILE` overrides) |
